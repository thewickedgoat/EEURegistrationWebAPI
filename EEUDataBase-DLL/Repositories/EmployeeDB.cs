...
{
    public class EmployeeDB : IDataBase<Employee, int>
    {
        private IContext context;

        public EmployeeDB(IContext context)
        {
        ...
        public bool Delete(int id)
        {
            using (var dbContext = GetContext())
            {
                var toBeDeleted = dbContext.Employees.Include("Absences").FirstOrDefault(x => x.Id == id);
                if (toBeDeleted != null)
                {
        ...
        public List<Employee> ReadAll()
        {
            using (var dbContext = GetContext())
            {
                return dbContext.Employees.Include("Department").Include("Absences").ToList();
            }
        }
        /*  
        ...
        public Employee ReadById(int id)
        {
            using (var dbContext = GetContext())
            {
                return dbContext.Employees.Include("Department").Include("Absences").FirstOrDefault(x => x.Id == id);
            }
        }
